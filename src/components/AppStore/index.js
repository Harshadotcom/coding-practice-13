...
class AppStore extends Component {
  ...
  updateApps = () => {
    ...
    const appResults = appsList.filter(
      eachVal => activeTabId === eachVal.category,
    )
    return appResults
  }

  render() {
    const {searchInputVal, activeTabId} = this.state
    const displayApps = this.updateApps()

    const searchResults = displayApps.filter(eachDisplayApp =>
      eachDisplayApp.appName
        .toLowerCase()
    ...
    return (
      <div className="web-page">
        <div className="bg-container">
          ...
          <ul className="unodered-tab-list">
            {tabsList.map(eachTab => (
              <TabItem
                tabDetails={eachTab}
                key={eachTab.tabId}
                updateStateVal={this.updateStateVal}
                isActive={eachTab.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="unodered-app-list">
